...
fn main() {
    ...
    if cfg!(feature = "intel-sycl") {
        ...
        println!("cargo:rustc-link-lib=parakeet");
...
fn get_whisper_cpp_version(whisper_root: &std::path::Path) -> std::io::Result<Option<String>> {
    ...
    for line in cmake_lists.lines() {
        ...
        // whisper.cpp ≥1.9 declares the version via WHISPER_VERSION_* vars.
        if let Some(value) = line
            .strip_prefix("set(WHISPER_VERSION_MAJOR ")
            .or_else(|| line.strip_prefix("set(WHISPER_VERSION_MINOR "))
            .or_else(|| line.strip_prefix("set(WHISPER_VERSION_PATCH "))
        {
            version_parts.push(value.trim_end_matches(')').trim().to_string());
